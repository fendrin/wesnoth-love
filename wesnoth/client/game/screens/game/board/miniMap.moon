...
setup = (dlg) ->
    ...
    minimap_scale_h = widget_h / (widget_h - tile_h)
    minimap_scale_w = widget_w / (widget_w - tile_w/1.5)
...
draw_ = (dlg) ->
    ...
    love.graphics.rectangle("line",
        0, 0, gameMap_w*minimap_scale, gameMap_h*minimap_scale
    )
...
local miniMap_Data, miniMap_image
draw = (dlg, gameMap_offset_x, gameMap_offset_y) ->

    unless drawn
        draw_(dlg)
        miniMap_Data  = miniMapScaled\newImageData!
        assert(miniMap_Data)
        miniMap_image = love.graphics.newImage(miniMap_Data)
        assert(miniMap_image)
        drawn = true
    ...
    -- love.graphics.draw(miniMapScaled,
        -- minimap_surface_x, minimap_surface_y)
    -- love.graphics.draw(miniMap_image,
    --     minimap_surface_x, minimap_surface_y)
    dlg.miniMap.icon = miniMap_Data
