...
rgb2hex = (red, green, blue) ->
    return (string.format("%x", bitwise.lshift(red, 16) +
        bitwise.lshift(green, 8) + blue))\upper!


recolor_image = ( surf, map_rgb ) ->
    if surf == nil
        return nil
    ...
    -- nsurf = love.image.newImageData(surf\getWidth!, surf\getHeight!, surf )
    -- if nsurf == nil
    --     print"failed to make neutral surface"
    --     return nil

    recolor = (x, y, r, g, b, alpha) ->
        -- Don't recolor invisible pixels.
        if alpha > 0

            hex = rgb2hex(r*255, g*255, b*255)
            if new_rgb = map_rgb[hex]
                nr, ng, nb = unpack(new_rgb)
                return nr/255, ng/255, nb/255, alpha

        return r, g, b, alpha

    -- nsurf\mapPixel(recolor)

    -- print surf\type!

    surf\mapPixel(recolor)
